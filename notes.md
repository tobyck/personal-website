...
All of the following information only applies to right angled triangles. The longest side of a triangle is called the hypotenuse (h), the side oposite to an angle marked (often the smaller one) is called the opposite side (o), and the side adjacent to the angle is called the adjacent side (a). An acronym to remember trigonometric functions is SOHCAHTOA — $\sin\theta = \frac{o}{h}$, $\cos\theta = \frac{a}{h}$, and $\tan\theta = \frac{o}{a}$. These eqautions can easily be rearranged to solve for θ, or any side length. Some examples are below:

To calculate the angle of a tringle with o = 3, a = 4, and h = 5, you can use any function because all of the side lengths are known. Say you use sine. You can rearrange $\sin(\theta) = \frac{o}{h}$ to say that $\theta = \arcsin(\frac{o}{h})$, by taking the arcsin (or $\sin^{−1}$) of each side. Therefore, $\theta = \arcsin(\frac{3}{5}) \approx 36\degree$

Now say you have the triangle a = 7, and θ = 30°, that's all you know. To find the side thength of o, you would rearrange $\tan\theta = \frac{o}{a}$ to $a \tan(\theta) = o$ by multiplyimg both sides of the equation by $a$. If you plug in the numbers, you get $o = a\space\tan(30) = 4.04\space(2dp)$  <br><br>
